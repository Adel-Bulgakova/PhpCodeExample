...
$(document).ready(function() {
	var table = $('#claims_comments').DataTable({
		ajax: {
			type: 'POST',
			url: 'index.php?route=proc_jsondata_get',
			data: {'query': 'claims_on_comments'},
...
function modal_stream_view(stream_uuid, stream_url, stream_name, view){
	if (view == "screen") {
		...
		$('.modal-body').html("<h4>" + stream_name + "</h4><br><div class=\"modal-screenshot\" style=\"background: url('/api/v1/streams/thumb/1/" + stream_uuid + "') center center no-repeat;  background-size: cover;\"></div>");
